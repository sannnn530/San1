...
// Make client available globally for web dashboard
...
process.on('unhandledRejection', error => {
    ...
    const embed = new Discord.EmbedBuilder()
        .setTitle(`ð¨ã»Unhandled promise rejection`)
        .addFields([
            {
                name: "Error",
...
process.on('warning', warn => {
    const embed = new Discord.EmbedBuilder()
        .setTitle(`ð¨ã»New warning found`)
        .addFields([
            {
                name: `Warn`,
                value: `\`\`\`${warn}\`\`\``,
...
client.on(Discord.ShardEvents.Error, error => {
    console.log(error)
    const errorStr = error ? String(error).slice(0, 1000) : 'Unknown error';
    const stackStr = error && error.stack ? String(error.stack).slice(0, 1000) : 'No stack trace';
    const embed = new Discord.EmbedBuilder()
        .setTitle(`ð¨ã»A websocket connection encountered an error`)
        .addFields([
            {
                name: `Error`,
                value: `\`\`\`${errorStr}\`\`\``,
